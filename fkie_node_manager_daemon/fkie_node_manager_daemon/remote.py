...
def open_channel(url, *, rosnode:[Node,None]=None):
    ...
    if security.has_keys():
        ...
        credentials = grpc.ssl_channel_credentials(root_certificates=security.get_ca_cert(), private_key=private_key, certificate_chain=certificate_chain)
        if rosnode is not None:
            rosnode.get_logger().debug("create secure channel to %s" % url)
        options = (('grpc.ssl_target_name_override', security.DEFAULT_COMMON_NAME,),)
    ...
    if time.time() - starttime > 5.0:
        if rosnode is not None:
            rosnode.get_logger().warn("Open insecure gRPC channel took too long (%.3f sec)! Fix your network configuration!" % (time.time() - starttime))
    return channel
