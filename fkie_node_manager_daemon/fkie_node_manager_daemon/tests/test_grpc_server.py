...
class TestGrpcServer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_include_file = interpret_path("$(find-pkg-share fkie_node_manager_daemon)/resources/include_dummy.launch")
#        self.assertEqual(self.test_include_file, "xx", "wrong count of directories in the root path, got: %s, expected: %s" % (self.test_include_file, "x"))
        path = interpret_path("$(find-pkg-share fkie_node_manager_daemon)/../../build")
    ...
    def test_get_included_files(self):
        ...
        return
        path = interpret_path("$(find-pkg-share fkie_node_manager_daemon)/tests/resources/include_dummy.launch")
...
#        for linenr, path, exists, file_list in inc_files:
    ...
    def _test_load_launch(self, unload=True):
        ...
        try:
            ...
            path = interpret_path("$(find-pkg-share fkie_node_manager_daemon)/resources/description_example.launch")
