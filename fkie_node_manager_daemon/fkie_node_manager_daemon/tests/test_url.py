...
class TestUrlLib(unittest.TestCase):
    ...
    def test_nmduri(self):
        ...
        self.assertEqual(grpc_exp, grpc_url, "wrong grpc url from default grpc uri, expected: %s, got: %s" % (grpc_exp, grpc_url))
        grpc_exp = 'grpc://%s:12321' % host.get_host_name()
        grpc_url = url.nmduri('')
        self.assertEqual(grpc_exp, grpc_url, "wrong grpc url from empty string, expected: %s, got: %s" % (grpc_exp, grpc_url))
