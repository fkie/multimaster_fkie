...
class TestCommonLib(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_include_file = "%s/%s/include_dummy.launch" % (self.nm_path, 'share/fkie_node_manager_daemon/resources')
    ...
    def test_package_name(self):
        ...
        self.assertEqual('', pkg[0], "wrong package name, expected: %s, got: %s" % ('', pkg[0]))
        self.assertEqual('', pkg[1], "wrong package name, expected: %s, got: %s" % ('', pkg[1]))
...
#        self.assertEqual(self.test_include_file, path, "wrong interpreted path with replace the subdirectory by `///`, expected: %s, got: %s" % (self.test_include_file, path))
        ...
        self.assertRaises(LookupError, interpret_path, text_path, "No rospkg.ResourceNotFound raises on invalid pacakge name")
    ...
    def test_replace_paths(self):
        ...
        path_exp = "%s/fkie_node_manager_daemon/share/fkie_node_manager_daemon/resources/include_dummy.launch, %s/fkie_node_manager_daemon/share/fkie_node_manager_daemon/launch/demo_bar.launch" % (nm_path, nm_path)
