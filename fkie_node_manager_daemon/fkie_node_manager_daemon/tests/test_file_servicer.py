...
from fkie_node_manager_daemon.common import get_pkg_path
...
class TestFileServiceServicer(unittest.TestCase):
    ...
    def setUp(self):
        self.current_pose = 0
        path = os.path.normpath(os.path.join(get_pkg_path(
            "fkie_node_manager_daemon"), "../../build"))
...
#        self.assertEqual(len(root_paths), len(launch_response.items), "ROS root paths are not equal, expected: %s, got: %s" % (root_paths, launch_response.items))
    ...
    def test_save_content(self):
        ...
        if save_response.status.code == fmsg.ReturnStatus.StatusType.Value('OK'):
            ...
            content.file.path = interpret_path(
                "$(find-pkg-share fkie_node_manager_daemon)/") + content.file.path
            save_response = next(fs.SaveFileContent([content], DummyContext()))
        self.assertEqual(save_response.status.code, fmsg.ReturnStatus.StatusType.Value(
            'IO_ERROR'), "save in root folder returns a wrong result: %d, expected: %d" % (save_response.status.code, fmsg.ReturnStatus.StatusType.Value('IO_ERROR')))
